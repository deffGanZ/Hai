...
require('./config')
const { default: hisokaConnect, useMultiFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@whiskeysockets/baileys")
...
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, reSize, fetchJson, await, sleep } = require('./lib/myfunc')
...
async function startHisoka() {
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./Gans")(hisoka, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    let fff = db.data.chats[anu.id].welcome
    if (!fff) return
        console.log(anu)
        try {
...
'displayName': `${num.split("@")[0]}`, 'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;${num.split("@")[0]},;;;\nFN:${num.split("@")[0]},\nitem1.TEL;waid=${num.split("@")[0]}:${num.split("@")[0]}\nitem1.X-ABLabell:Ponsel\nEND:VCARD`, 'jpegThumbnail': {url:ppuser}, thumbnail: {url:ppuser},sendEphemeral: true}}}

                if (anu.action == 'add') {
hisoka.sendMessage(anu.id, { text : `Welcome To ${metadata.subject} @${num.split("@")[0]}`, 
mentions: [num], 
}, { quoted: fkont })
                } else if (anu.action == 'remove') {
hisoka.sendMessage(anu.id, { text : `@${num.split("@")[0]} Leaving To ${metadata.subject}`, 
mentions: [num], 
}, { quoted: fkont })
